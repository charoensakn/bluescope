// biome-ignore-all lint/suspicious/noExplicitAny: test file uses any for mock handlers
...
describe('browser module', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(ipcMain.handle).mockImplementation(
      (channel: string, handler: (event: Electron.IpcMainInvokeEvent, ...args: any[]) => any) => {
        handlers[channel] = handler;
        return {} as Electron.IpcMain;
      },
    );
    vi.mocked(ipcMain.on).mockImplementation(
      (channel: string, handler: (event: Electron.IpcMainEvent, ...args: any[]) => void) => {
        handlers[channel] = handler;
        return {} as Electron.IpcMain;
      },
    );
